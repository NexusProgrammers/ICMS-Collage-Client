...
import { resendForgotPasswordCode, verifyCode } from "../service/authService";
...
const VerifyCodeModal = ({ isOpen, onClose }) => {
  ...
  const { verifyCodeLoading } = useSelector((state) => state.auth);
  const { isDarkMode } = useTheme();
  const { resendForgotPasswordCodeLoading } = useSelector(
    (state) => state.auth
  );
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await dispatch(
      dispatch(verifyCode({ code, navigate, _id }))
    );
  ...
  const resendOTP = () => {
    dispatch(resendForgotPasswordCode());
  };

  return (
    <>
      {verifyCodeLoading && <Spinner />}
      <Modal
        open={isOpen}
      ...
      >
        ...
        >
          ...
          <div className="flex flex-row items-center justify-center text-center text-sm font-medium space-x-1 text-gray-500 py-6">
            ...
            {resendForgotPasswordCodeLoading ? (
